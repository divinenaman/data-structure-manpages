...
class SegmentTrees{
    public:
        ...
        SegmentTrees(T* arr,int size){
            ...
            this->end = size-1;
            this->vertex = 1;
            this->segment.resize(this->size+1);
        ...
        T find(int start,int end,int v,int l,int h){
            if(l>h) return 0;
            if(l == start && h == end) return this->segment[v];
            int mid = (start+end)/2;
        ...
        void update(int start,int end,int v,int pos,T val){
            if(start == end && start == pos) this->segment[v] = val;
            else{
                int mid = (start+end)/2;
                if(pos<=mid) update(start,mid,v*2,pos,val);
                else update(mid+1,end,v*2+1,pos,val);
